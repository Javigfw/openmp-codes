...
/*---------------------------------------------------------------------------*/
//TODO Add pRandData parameters at the end HECHO
...
/*---------------------------------------------------------------------------*/
//TODO Add pRandData  parameter at the end HECHO
                     ...
                     const int SiEnergy, const int SeFEnergy, struct drand48_data *pRandData)	
{
 ...
 int NSharksLocal = 0;
 # ifdef _OPENMP
 #pragma omp parallel for default(none) shared(Rows, Cols, Ocean, SimIter, NiFBreed, NiSBreed, SiEnergy, SeFEnergy, pRandData) reduction(+:NFishLocal) reduction(+:NSharksLocal) schedule(static)
 #endif
     ...
     for (int j=0; j<Cols; j++)
         {
          if (Ocean[i][j]!=NULL && Ocean[i][j]->Animal==FISH)
             //TODO Add pRandData parameters at the end HECHO
             IterateFish(Ocean,Rows,Cols,i,j,SimIter, &NFishLocal,NiFBreed, pRandData);
          if (Ocean[i][j]!=NULL && Ocean[i][j]->Animal==SHARK)
             //TODO Add pRandData  parameters at the end HECHO
             IterateShark(Ocean,Rows,Cols,i,j,SimIter,
                          &NFishLocal, &NSharksLocal, NiSBreed,
