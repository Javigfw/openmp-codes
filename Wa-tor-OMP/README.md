...
* Mayoría en GTA1, GTA2 o GTA3  : GT1
...
No, porque cada instancia que generemos de números aleatorios(movimientos) será diferente en cada ejecución.
...
No, porque cada generación de numeros aleatorios es diferente para cada hilo, por lo que no obtendran la misma carga de trabajo.
...
Static, porque no hay diferencia de coste de trabajo entre un bit u otro de la matriz.
...
No, porque no podemos comparar los tiempos con distintos hilos debido a que con 2 hilos y con 4 se genera una carga de trabajo distinta.

9. **¿Son razonables los tiempos alcanzados? ¿Porqué?**
Son razonables, pero aún así seguimos sin poder compararlos ya que no están realizando el mismo trabajo entre distintos hilos
...
Más fallos de caché, ya que no se accederían a todas las posiciones que se cargan en memoria ya que el programa paralelo va dando "saltos" hacia zonas de memoria distintas.
No, porque solo reparte iteraciones y puede añadir overhead
