...
if (!ExistArg("-ffmpeg",argc,argv))
 ...
 //TODO Get memory for pRandData, with size nthreads. Possible False Sharing. HECHO
 pRandData = (struct drand48_data *) GetMem(NThreads, sizeof(struct drand48_data), "Main:pRandData");
 //TODO: init seed for each  drand48_data using sdrand48_r(i,...), i=0..nthreads-1. See man pages. HECHO
 for(int i = 0; i < NThreads; i++){
 	long seed = 0 + i*10;
 	srand48_r(seed, &pRandData[i]);
 }
 #endif
 ...
 while (SimIter < MaxNIter && NFishes >0 && NSharks > 0)
       {
        ...
        IterateOcean((struct StAnimal ***)Ocean,Rows,Cols,SimIter,&NFishes,&NSharks,
 ...
 //TODO Free mem for pRandData HECHO
 free(pRandData);
